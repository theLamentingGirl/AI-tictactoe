...
#bestPos returns the best position after running the minmax algorithm
                if np.isnan(self.gameArea[i,j]): # if its empty
                    ...
                    self.gameArea[i,j]=np.nan#needed to empty that spot so that real bestPos can be feeded
...
#        self.player=1
        ...
        if not(winStatus[0]):
            ...
            print(self.player)
            didhe=self.prompt(self.player,self.bestMove()) #changes from CompvsUser
